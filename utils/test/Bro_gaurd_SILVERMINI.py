# filename: Bro_gaurd_SILVERMINI.py
...
DATA_FILE = "historical_data.csv"   # change if needed
# DATA_FILE = "angelone_candles.csv"   # change if needed
...
def load_data(path: str) -> pd.DataFrame:
    ...
    if dropped > 0:
        print(f"⚠️ Dropped {dropped} rows with missing OHLC/datetime.")
...
def safe_save_csv(df: pd.DataFrame, filename: str):
    try:
        ...
        print(f"⚠️ {filename} is locked (open in another program). Saved instead as → {alt}")
...
def backtest_signal(df):
    """
    Takes full dataframe till current candle
    Returns: BUY / SELL / HOLD
    """
    last = df.iloc[-1]

    # COPY the exact logic from your backtest
    # if last["ema_27"] > last["ema_78"] and last["close"] > last["supertrend"]:and last["close"] < last["supertrend"]
    if last["ema_27"] > last["ema_78"]:
        return "BUY"
...
def save_balance_chart(events_df):
    ...
    if curve.empty:
        print("ℹ️ Not enough EXIT events to plot balance curve.")
        return
